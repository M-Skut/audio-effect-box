...
void Menu::redrawSettings() {
	...
	for (uint8_t i = 0; i < 5; i++) {
		m_canvas.drawRect(2 + (25 * i), 1, 25 + (25 * i), 2);
		m_canvas.drawRect(2 + (25 * i), 4, 25 + (25 * i), 5);
	}
	m_canvas.setColor(WHITE);
	// Color currently enabled boxes
	if (leds_enabled)
		m_canvas.drawRect(2, 4, 25, 5);
	if (delay_enabled)
		m_canvas.drawRect(26, 4, 50, 5);
	if (distortion_enabled)
		m_canvas.drawRect(51, 4, 75, 5);
	if (tremolo_enabled)
		m_canvas.drawRect(76, 4, 100, 5);
	if (filter_enabled)
		m_canvas.drawRect(101, 4, 125, 5);

	// Color currently selected menu item
	if (current_menu_item == LED_CONTROL)
		m_canvas.drawRect(2, 1, 26, 2);
	if (current_menu_item == DELAY_CONTROL)
		m_canvas.drawRect(27, 1, 51, 2);
	if (current_menu_item == DISTORTION_CONTROL)
		m_canvas.drawRect(52, 1, 76, 2);
	if (current_menu_item == TREMOLO_CONTROL)
		m_canvas.drawRect(77, 1, 101, 2);
	if (current_menu_item == FILTER_CONTROL)
		m_canvas.drawRect(102, 1, 126, 2);
...
void Menu::processMenu() {
	...
	case IDLE: {
		...
		} else if (buttons_data & 0x4) { // Move manu to right
			if (!debouncePin(0x4))
				return;
			changeSelection(true);

			current_menu_state = BUTTON_PRESSED;
		} else if (buttons_data & 0x8) { // Move manu to left
			if (!debouncePin(0x8))
				return;
